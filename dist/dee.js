...
(function() {
  ...
  d.reactive = function(fn, options) {
    ...
    rfn.run = function() {
      ...
      t = timer();
      debug("context started >", this.id);
      this.trigger("run:before");
      fn.call(rfn);
      this.trigger("run");
      this.trigger("run:after");
      debug("context finished >", this.id, t());
      self.current = old;
  ...
  d.event = function(path) {
    return function() {
      var args, attr, dp, event, fp, isBase, model, name, t, _ref;
      ...
      if (_.contains(["add", "remove", "reset", "sort"], name)) {
        ...
        if (!((model = args[0]) instanceof Backbone.Model)) {
          return;
        }
        d.trigger(d.join(fp, model.cid), "change", {
          isBase: false
        });
      }
      return debug("event end >", "" + name + ":" + fp, t());
    };
  };
  ...
  d.unbind = function(event, fn) {
    ...
    if (!_.isFunction(fn)) {
      ...
      evts[name] = _.filter(evts[name], function(s) {
        return s.fn !== fn;
      });
      if (_.isEmpty(evts[name])) {
        return delete evts[name];
      }
    }
  };
  ...
  d.exec = function(event, attr, options) {
    var evts, path, subs, t, val;
    t = timer();
    if (!_.isObject(evts = d._paths[attr])) {
      return;
    }
    if (!_.isArray(subs = evts[event])) {
      return;
    }
    val = d.retrieve(d.split(attr));
    path = options.path || attr;
    _.each(subs, function(s) {
      var ctx;
      ctx = s.context || null;
      return s.fn.call(ctx, val, path, options);
    });
    return debug("exec > " + event + ":" + attr + " (x" + subs.length + ")", t());
  };

  d.trigger = function(attr, name, options) {
    var base, evts, exec, parts, path, subpath, _ref, _ref1;
    if (name == null) {
      name = "change";
    }
    if (options == null) {
    ...
    exec = function(name, path) {
      return d.exec(name, path, options);
    };
...
},{"underscore":2,"backbone-deep-model":3}],2:[function(require,module,exports){
...
},{"backbone":5,"underscore":2}],5:[function(require,module,exports){
